/* массив точек пути будет иметь следующий вид:
    [
    {x: координата, y: координата},
    {x: координата, y: координата},
    ...
    ]
*/

// создаем массив точек, расположенных буквой "Г"
function getSvgSize() {
    const svg = d3.select("svg");
    const { width, height } = svg.node().getBoundingClientRect();

    return { width, height };
}

function createPathG() {
    const { width, height } = getSvgSize();

    let data = [];
    const padding = 100;
    
    // начальное положение рисунка
    let posX = padding;
    let posY = height - padding;
    const h = 5;

    // координаты y - уменьшаются, x - постоянны
    while (posY > padding) {
        data.push( {x: posX, y: posY});
        posY -= h;
    }
    
    // координаты y - постоянны, x - увеличиваются
    while (posX < width - padding) {
        data.push( {x: posX, y: posY});
        posX += h;
    }

    return data
}

// создаем массив точек, расположенных по кругу
function createPathCircle() {
    const { width, height } = getSvgSize();

    let data = [];

    // используем параметрическую форму описания круга
    // центр асположен в центре svg-элемента, а радиус равен трети высоты/ширины
    for (let t = 0; t <= Math.PI * 2; t += 0.1) {
        data.push(
            {x: width / 2 + width / 4 * Math.sin(t),
            y: height / 2 + height / 4 * Math.cos(t)}
        );
    }

    return data
}

function createGypocicle() {
    const { width, height } = getSvgSize();

    let data = [];
    const centerX = width / 2;
    const centerY = height / 2;
    const outerRadius = Math.min(width, height) * 0.35;
    const innerRadius = outerRadius / 5;

    for (let t = 0; t <= Math.PI * 2; t += 0.02) {
        data.push(
            {
                x: centerX + (outerRadius - innerRadius) * Math.cos(t) +
                    innerRadius * Math.cos(((outerRadius - innerRadius) / innerRadius) * t),
                y: centerY + (outerRadius - innerRadius) * Math.sin(t) -
                    innerRadius * Math.sin(((outerRadius - innerRadius) / innerRadius) * t)
            }
        );
    }

    return data
}

// создаем путь и отображаем его в svg-элементе
const drawPath =() => {
    const svg = d3.select("svg")

    const dataPoints = createGypocicle();

    const line = d3.line()
        .x((d) => d.x)
        .y((d) => d.y);

    // создаем путь на основе массива точек
    const path = svg.append('path')
        .attr('d', line(dataPoints))
        .attr('stroke', 'black')
        .attr('fill', 'none');

    return path;
}

function translateAlong(path, options = {}) {
    const {
        sxFrom = 1,
        sxTo = 1,
        syFrom = 1,
        syTo = 1,
        angleFrom = 0,
        angleTo = 0
    } = options;

    if (!path || !path.getAttribute("d")) {
        return function() {
            return function() {
                return `translate(0,0) scale(${sxFrom},${syFrom}) rotate(${angleFrom})`;
            }
        }
    }

    const length = path.getTotalLength();
    return function() {
        return function(t) {
            const {x, y} = path.getPointAtLength(t * length);
            const sx = sxFrom + (sxTo - sxFrom) * t;
            const sy = syFrom + (syTo - syFrom) * t;
            const angle = angleFrom + (angleTo - angleFrom) * t;

            return `translate(${x},${y}) scale(${sx},${sy}) rotate(${angle})`;
        }
    }
}
